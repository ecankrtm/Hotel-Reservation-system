...
namespace Bade_Butik_Otel_Rezervasyon_Otomasyonu
{
    public partial class FrmYeniMusteri : Form
    {
        ...
        {
            baglanti.Open();
            SqlCommand komut = new SqlCommand("insert into MusteriEkle(Adi,Soyadi,Cinsiyet,Telefon,Mail,TC,OdaNo,Ucret,GirisTarihi,CıkısTarihi) values('" + TxtAdi.Text + "','" + TxtSoyadi.Text + "','" + comboBox1.Text +"','"+MskTxtTelefon.Text+"' ,'"+TxtMail.Text+"','"+TxtKimlikNo.Text+"','"+TxtOdaNo.Text+"','"+TxtUcret.Text+"','"+DtpGirisTarihi.Value.ToString("yyyy-MM-dd")+"','"+DtpCıkısTarihi.Value.ToString("yyyy-MM-dd")+"')", baglanti);
            komut.ExecuteNonQuery();
            baglanti.Close();
...
//Data Source=ABDURRAHMAN\SQLEXPRESS;Initial Catalog=BadeButikOtel;Integrated Security=True;Trust Server Certificate=True